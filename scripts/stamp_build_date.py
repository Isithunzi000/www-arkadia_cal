...
Katalog zrodlowy z arkadia_cal/cal.js (zrodlo prawdy w zipach;
...
# Naglowek komentarza "// arkadia_cal vX.Y.Z | DD-MM-YYYY": wersja z
# EXT_VERSION w tym samym pliku, data = dzis. Wzorzec musi trafic
# dokladnie raz, inaczej exit 1.
HEADER_REL = 'arkadia_cal/cal.js'
HEADER_RX = re.compile(r"^// arkadia_cal v[\d.]+ \| \d{2}-\d{2}-\d{4}$", re.M)
...
def stamp_header(root):
    ...
    new = "// arkadia_cal v%s | %s" % (m.group(1), TODAY)
    if hits[0] == new:
