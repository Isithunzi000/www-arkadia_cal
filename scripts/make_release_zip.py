#!/usr/bin/env python3
"""Deterministyczny build zipa release'owego rozszerzenia arkadia_cal (WWW).

Wejscie: katalog zrodlowy zawierajacy arkadia_cal/{cal.js,manifest.json}
(zrodlo prawdy zyje w zipach w releases/; ten skrypt je odtwarza).
Wyjscie: releases/arkadia_cal_<wersja>.zip — wpisy sortowane, timestampy
sztywne (1980-01-01), stale uprawnienia, STORED; podwojny build =
identyczny SHA-256.

Uzycie:
  python3 scripts/make_release_zip.py <katalog_zrodlowy> <wersja>
Przyklad (nowa wersja 1.0.9):
  mkdir src && cp -r arkadia_cal src/   # po edycji cal.js/manifest.json
  python3 scripts/make_release_zip.py src 1.0.9
  -> releases/arkadia_cal_1_0_9.zip; potem: git add releases/ && push
