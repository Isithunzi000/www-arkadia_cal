...
// EXT_DATE, ale tez linie naglowka "// arkadia_cal vX.Y.Z | DD-MM-YYYY"
...
function makeFixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'cal-stamp-'));
...
test('stamp: naglowek dostaje wersje z EXT_VERSION i dzisiejsza date', () => {
  ...
  fs.writeFileSync(path.join(root, PKG, 'cal.js'), CAL_JS);
  const r = runStamp(root);
  assert.equal(r.status, 0, r.stderr);
  const out = fs.readFileSync(path.join(root, PKG, 'cal.js'), 'utf8');
...
test('stamp: re-run tego samego dnia = no-op (idempotentnosc)', () => {
  ...
  fs.writeFileSync(path.join(root, PKG, 'cal.js'), good);
  const r = runStamp(root);
  assert.equal(r.status, 0, r.stderr);
  assert.equal(fs.readFileSync(path.join(root, PKG, 'cal.js'), 'utf8'), good,
